...
#define GPIO_NUM 8
// gpio-id->gpio-name
const char* NAMES[GPIO_NUM] = {"160", "161", "162", "163", 
"164", "165", "166", "57"};
...
{
	write_str("/sys/class/gpio/export", NAMES[id]);
}

static void unexport(unsigned int id)
{
	write_str("/sys/class/gpio/unexport", NAMES[id]);
}

static void set_dir(unsigned int id, const char dir[])
{
	char buf[128];
	sprintf(buf, "/sys/class/gpio/gpio%s/direction", NAMES[id]);
	write_str(buf, dir);
}
...
{
	char buf[128], buf2[4];
	sprintf(buf, "/sys/class/gpio/gpio%s/value", NAMES[id]);
